	...
	void Update () {
		...
		if(transform.position.y==-3f)
		{
           ...
           /* Vector3 paddlePos = new Vector3(8.0f, this.transform.position.y, 0.0f);
			...
			if(touchRay.GetPoint(1).x<PlayerPrefs.GetFloat("right") && touchRay.GetPoint(1).x>(-1)*PlayerPrefs.GetFloat("right") && touchRay.GetPoint(1).y<=-2)
			{
				transform.position = new Vector2(touchRay.GetPoint(1).x, transform.position.y);
			}
